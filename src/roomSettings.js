...
export default class RoomSettings extends Component {
  ...
  roleToPowerLevel = role => role === 'admin'
    ? 100
    : role === 'mod'
      ? 50
  ...
  handleSubmit = async e => {
    ...
    if (this.state.readability !== this.initialReadability) {
      await Client.client.sendStateEvent(this.props.room.roomId, Matrix.EventType.RoomHistoryVisibility, {
...
class ConfigurePowerForState extends Component {
  constructor (props) {
    ...
    this.initialRole = this.initialPowerLevel >= 100
      ? 'admin'
      : this.initialPowerLevel >= 50
        ? 'mod'
  ...
  render (props) {
    ...
    return <Fragment>
        ...
        <div class="room-settings-info">
          {currentRole === 'admin'
            ? `Only admins can ${props.act}`
            : currentRole === 'mod'
              ? `Admins and moderators can ${props.act}`
              : currentRole === 'member'
...
class OtherRoleList extends Component {
  getOtherRoles = _ => {
    const others = []
    for (const user in this.props.users) {
      if (this.props.users[user] !== 100 && this.props.users[user] !== 50) { others.push(<div class="room-settings-otherrole-listing" key={user}>{user}</div>) }
    }
