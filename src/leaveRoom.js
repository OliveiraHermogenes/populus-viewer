...
export default class LeaveRoom extends Component {
  ...
  deepForget = async _ => {
    ...
    await new Promise(resolve => this.setState({ progress: 0, childTotal: theChildren.length }, resolve))
    for (const child of theChildren) {
      await Client.client.leave(child.getStateKey())
      await Client.client.forget(child.getStateKey())
      await new Promise(resolve => this.setState(oldState => ({ progress: oldState.progress + 1 }), resolve))
    }
    this.forgetRoom()
  }
