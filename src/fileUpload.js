...
export default class FileUpload extends Component {
  ...
  validateFile = () => {
    ...
    const fileValid = this.validateFileType(theFile.type) && !(theFile.size >= limit)
  ...
  uploadHandler = async e => {
    ...
    const roomId = this.state.fileValid ? await this.uploadFile() : await this.uploadExternalLink()
    const theRoom = await Client.client.getRoomWithState(roomId)
  ...
  uploadFile = async () => {
    ...
    const response = await Client.client.uploadContent(theFile, { progressHandler: this.progressHandler })
      .catch(this.uploadError)
    const { room_id: roomId } = await Client.client.createRoom({
