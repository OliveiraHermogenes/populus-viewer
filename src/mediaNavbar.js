...
export default class MediaNavbar extends Component {
  ...
  zoomIn = _ => this.props.content.current.setZoom(this.props.content.current.zoomFactor + 0.1)

  zoomOut = _ => this.props.content.current.setZoom(this.props.content.current.zoomFactor - 0.1)
...
class Progress extends Component {
  ...
  render (props) {
    ...
    return <div class="nav-position">
      <span ref={this.timeCurrent} style={{ width: `${timeStamp.length + 0.5}ch` }} id="nav-time-elapsed">{timeStamp}</span>
      <span>/</span>
      <span ref={this.timeTotal} style={{ width: `${timeTotal.length + 0.5}ch` }} id="nav-time-total">{timeTotal}</span>
    </div>
  }
}
